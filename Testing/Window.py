...
def create_numpad(win):
    ...
    for key in range(1,10):
        Button(num_keys, text=f"{key}", font=button_font).grid(column=x, row=y, stick='nsew')
        x+=1
        if x%3==0:
            x=0; y+=1
    Button(num_keys, text="0", font=button_font).grid(column=x+1, row=y, stick='nsew')
    ...
    Button(control_keys, text="Accept", font=(None, 10), bg="lightgreen").grid(column=0, row=0, stick='nsew')
    Button(control_keys, text="Back", font=(None, 10), bg="orange").grid(column=0, row=1, stick='nsew')
    Button(control_keys, text="Cancel", font=(None, 10), bg="red").grid(column=0, row=2, stick='nsew')
...
if __name__ == "__main__":
    ...
    win = create_window()
    screen(win)
    Button(win, text='Clear it', command= lambda: clear_window(win)).place(relx=0.5, rely=0.05, anchor='n', width=100)
    Button(win, text='Exit', command= exit).place(relx=0.5, rely=0.1, anchor='n', width=100)
    create_numpad(win)
    win.mainloop()
